...
foldConstrainedList :: Monoid r => (forall a. c a => a -> r) -> ConstrainedList c -> r
...
-- Here we need UndecidableInstances, because the compiler has no guarantee
-- that trying to prove a is a Monoid or a Show is decidable (finishes).
-- a can be anything, not necessarily something closer to an atomic structure
-- that has a non recursive definition of Monoid and Show.
...
-- We can now show everything in the list, as well as fold a list with
-- its own Monoid instance.
...
-- Type family for mapping constraints
...
foldHList :: (Every c a, Monoid r) => TCProxy c
                                   -> (forall x. c x => x -> r)
                                   -> HList a
                                   -> r
foldHList _ _ HNil = mempty
foldHList p f (HCons x xs) = f x <> foldHList p f xs
...
-- It complains that c is ambiguous. Without The constraint kind, i.e without
-- an object to encapsulate the constraint, it cannot be resolved on the fct
-- level.
...
foldMapH f (HCons x xs) = f x `mappend` foldMapH f xs
